...
>
> `[insert screenshot]`<img width="406" height="146" alt="Screenshot_1" src="https://github.com/user-attachments/assets/e298f1e6-c8cb-4ca5-b068-a60c64583910" />
...
>
> `[insert screenshot]`<img width="232" height="343" alt="Screenshot_2" src="https://github.com/user-attachments/assets/2ab1494a-79ec-49b1-afaa-8d4c7cde23ec" />
...
>
> `[insert screenshot]`<img width="934" height="100" alt="Screenshot_3" src="https://github.com/user-attachments/assets/a90db4d1-fe2d-4c24-b299-d3d1f57a9f13" />
...
>
> `[insert screenshot]`<img width="665" height="250" alt="Screenshot_4" src="https://github.com/user-attachments/assets/be8c786e-4624-4b3d-b9a5-d7f8c0693992" />
...
> insert it here.
>
> `[insert screenshot]`<img width="929" height="440" alt="Screenshot_5" src="https://github.com/user-attachments/assets/5f55f290-a0ca-478b-b18c-37437d8cd0d6" />
...
> *Your answer:* The "Install PlantUML" step takes significantly longer (often 20-30 seconds), while rendering the SVG takes only 1-2 seconds. This suggests that optimization efforts should be directed at bypassing the apt-get installation step entirely (e.g., using a custom Docker image).
...
> *Your answer:* Yes, they are perfectly identical. This proves the reproducibility of the build process: anyone (or any CI server) compiling the exact same plain-text source file will consistently generate the exact same artifact.
...
> *Your answer:* With binary files like .drawio or .png, Git only registers that the file has changed, but it cannot show what changed inside it. We would completely lose the readable text diff, making it impossible to see if a cardinality was modified or an entity was added just by looking at the commit history.
...
> *Your answer:* No, this would not be possible with a binary file. Git would throw a binary merge conflict, and one person's work would have to completely overwrite the other's. Because PlantUML is text-based, Git can seamlessly auto-merge changes on different lines, enabling smooth collaboration for the team.
...
> *Your answer:* An annotated tag is an immutable pointer to a specific point in time and contains locked release metadata (author, date, message). A branch is a moving pointer that changes with every new commit. A tag is used for an unchangeable snapshot (like a final release version), while a branch is used for continuous ongoing development.
...
> *Your answer:* The CI pipeline completely eliminates the manual steps of rendering the diagram, exporting the image, and manually uploading it to a release page. Instead, it provides a strict guarantee that the published image is always automatically, perfectly synchronized with the underlying source code.
...
>
> `[insert screenshot]`<img width="878" height="428" alt="Screenshot_6" src="https://github.com/user-attachments/assets/c4777ed5-836e-44bc-b7de-ef5f9351bb41" />
...
4. **Own extension:** Add an entity of your choice to the schema – for example
   ...
   >
   > `[insert screenshot]`<img width="738" height="358" alt="Screenshot_7" src="https://github.com/user-attachments/assets/4da8b022-eceb-4103-ac5d-58541c5b7b72" />
